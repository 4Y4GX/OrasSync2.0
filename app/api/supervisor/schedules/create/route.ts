// app/api/supervisor/schedules/create/route.ts
...
export async function POST(request: Request) {
  try {
    ...
    // Check if user exists and is accessible (for supervisors, check if they're in their team)
    ...
    if (user.role_id === 2 && targetUser.supervisor_id !== user.user_id) {
    ...
    await prisma.d_tblaudit_log.create({
      data: {
        ...
        old_value: null,
        new_value: `Created schedule for user: ${user_id}`,
      },
    });
